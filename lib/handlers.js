                    ...
                    _data.delete('checks', checkId, function (err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {
                            Error:
                              "Errors encountered while attempting to delete all of the user's checks. All check may not have been deleted from the system successfully.",
                          });
                        }
                      }
                    });
  ...
  if (id) {
    ...
    _data.read('tokens', id, function (err, tokenData) {
      if (!err && tokenData) {
        ...
        callback(400, { Error: 'Could not find the specified token' }); // Not found
      }
    });
  } else {
    callback(400, { Error: 'Missing required field' });
...
// Veriify if a given token id is currently valid for a given user
